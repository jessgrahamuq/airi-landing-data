...
(function () {
  ...
  // How each coverage segment is rendered relative to the parent domain color
  var COVERAGE_STYLE = {
  ...
  function render(mount, data) {
    ...
    function showTooltip(code, evt) {
      ...
      var rows = levels.map(function (lvl) {
        var c = row[lvl] || 0;
        return '<div class="airi-gchart-tt-row">' +
          '<span class="airi-gchart-tt-label">' +
          '<span class="airi-gchart-tt-swatch" style="background:' + color + ';opacity:' + COVERAGE_STYLE[lvl].opacity + '"></span>' +
          esc(COVERAGE_STYLE[lvl].label) + '</span>' +
          '<span class="airi-gchart-tt-count">' + c + '</span>' +
          '</div>';
      }).join('');
      ...
      tooltipEl.innerHTML =
        ...
        rows +
        '<div class="airi-gchart-tt-row airi-gchart-tt-total">' +
        '<span>Total</span><span>' + row.total + '</span></div>' +
        hint;
      ...
      if (!items.length) {
        ...
        list = '<ul class="airi-gchart-modal-list">' + items.map(function (it) {
          ...
          var levelBadge = it.level
            ? '<span class="airi-gchart-modal-item-level">' +
              '<span class="airi-gchart-modal-item-dot" style="background:' + domColor + ';opacity:' + (COVERAGE_STYLE[it.level] ? COVERAGE_STYLE[it.level].opacity : 1) + '"></span>' +
              esc(it.level + ' coverage') + '</span>'
            : '';
          var metaLine = metaBits.join(' <span>\u00b7</span> ');
          return '<li class="airi-gchart-modal-item">' +
            '<a class="airi-gchart-modal-link" href="' + esc(it.url) + '" target="_blank" rel="noopener noreferrer">' +
            '<div class="airi-gchart-modal-item-title">' + esc(it.title) + '</div>' +
            '<div class="airi-gchart-modal-item-meta">' +
            levelBadge +
            (levelBadge && metaLine ? ' <span>\u00b7</span> ' : '') +
            metaLine +
            '</div></a></li>';
        }).join('') + '</ul>';
      }
